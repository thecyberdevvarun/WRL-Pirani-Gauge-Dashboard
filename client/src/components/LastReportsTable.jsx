  ...
  return (
    <section className="mt-5 mx-4 bg-white border rounded-xl shadow mb-6">
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <span className="font-semibold text-slate-700 font-display">Last 10 Test Results</span>
        <Link to="/reports" className="text-xs text-emerald-700 hover:underline">
          View all →
        </Link>
      </div>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              ...
              >
                ...
                <td className={`px-3 py-2 text-center font-bold ${RESULT_CLASS[r.final_result] || "text-orange-500"}`}>
                  {r.final_result || "—"}
                </td>
              </tr>
            ))}
